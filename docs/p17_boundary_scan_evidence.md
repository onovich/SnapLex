# P17 Boundary Scan Evidence

Date: 2026-07-17
Phase: P17 Limited Credential Package Pilot And Signing Decision
Status: boundary, artifact, secret, and no-network scans PASS

P17 Round 9 confirms that limited credential package pilot planning preserved
the accepted runtime boundaries and repository hygiene. Local package outputs
may exist from package smoke, but they remain ignored and untracked.
...
rg -n "api_key=" . -g "!docs/p11_boundary_scan_evidence.md" -g "!docs/p12_boundary_scan_evidence.md" -g "!docs/p13_boundary_scan_evidence.md" -g "!docs/p14_boundary_scan_evidence.md" -g "!docs/p15_boundary_scan_evidence.md" -g "!docs/p16_boundary_scan_evidence.md"
rg -n 'API_KEY="' .
rg -n "API_KEY='" . -g "!docs/p11_boundary_scan_evidence.md" -g "!docs/p12_boundary_scan_evidence.md" -g "!docs/p13_boundary_scan_evidence.md" -g "!docs/p14_boundary_scan_evidence.md" -g "!docs/p15_boundary_scan_evidence.md" -g "!docs/p16_boundary_scan_evidence.md"
...
- no `api_key=` matches outside historical boundary-scan command text;
...
Runtime code and tests continue using the phase-neutral
`snaplex/package-credential-smoke` reference and
`SnapLexPackageCredentialSmoke` service.

## UI, Provider, And Credential Boundary Scan
...
Result: PASS with expected UI-only keyring text/control matches in
`snaplex\ui\app_shell.py`.

Observed matches are limited to Settings labels, source selection, and control
enablement. UI code does not call provider classes, HTTP clients, or direct
keyring `set_password`, `get_password`, or `delete_password` APIs.
...
Result from Round 5: expected rejection PASS with
...
Result from Round 5: expected rejection PASS.

Real-provider network smoke was not run in P17 because no existing local
...
```cmd
python -m pytest tests\test_release_smoke.py tests\test_trial_readiness.py tests\test_credentials.py --basetemp tmp\pytest-p17-boundary
```
...
- The scan covers generated artifacts, token/key patterns, old P15 smoke names,
...
- No generated outputs, screenshots, local data, keyring exports, `.env`, logs,
  OCR caches, smoke data, tester personal data, package outputs, or secrets are
  committed.
