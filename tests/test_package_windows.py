...
def test_build_pyinstaller_command_uses_snaplex_entrypoint() -> None:
...
def test_build_pyinstaller_command_supports_windowed_mode(tmp_path: Path) -> None:
    command = build_pyinstaller_command(
        PackageWindowsOptions(
...
def test_package_windows_dry_run_prints_command(capsys) -> None:
    ...
    assert subprocess.list2cmdline([str(ENTRY_SCRIPT)]) in output
